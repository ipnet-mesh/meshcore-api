...
class Config:
    ...
    def from_args_and_env(cls, cli_args: Optional[dict] = None) -> "Config":
        ...
        conn_group.add_argument(
            "--serial-port", type=str, help="Serial port device (e.g., /dev/ttyUSB0)"
        )
        conn_group.add_argument("--serial-baud", type=int, help="Serial baud rate")
        conn_group.add_argument(
            "--use-mock", action="store_true", help="Use mock MeshCore instead of real device"
        )
        conn_group.add_argument("--mock-scenario", type=str, help="Mock scenario name for playback")
        conn_group.add_argument(
            "--mock-loop", action="store_true", help="Loop mock scenario indefinitely"
        )
        conn_group.add_argument(
            "--mock-nodes", type=int, help="Number of simulated nodes in random mode"
        )
        conn_group.add_argument(
            "--mock-min-interval",
            type=float,
        ...
        conn_group.add_argument(
            "--mock-center-lat", type=float, help="Center latitude for simulated nodes"
        )
        conn_group.add_argument(
            "--mock-center-lon", type=float, help="Center longitude for simulated nodes"
        )

        # Database arguments
        db_group = parser.add_argument_group("Database")
        db_group.add_argument("--db-path", type=str, help="SQLite database file path")
        db_group.add_argument("--retention-days", type=int, help="Data retention period in days")
        db_group.add_argument(
            "--cleanup-interval-hours", type=int, help="Cleanup task interval in hours"
        )

        # API arguments
        api_group = parser.add_argument_group("API")
        api_group.add_argument("--api-host", type=str, help="API server host")
        api_group.add_argument("--api-port", type=int, help="API server port")
        api_group.add_argument("--api-title", type=str, help="API title for OpenAPI documentation")
        api_group.add_argument("--api-version", type=str, help="API version")
        ...
        write_group.add_argument(
            "--no-write", action="store_true", help="Disable write operations (read-only mode)"
        )
        ...
        webhook_group.add_argument(
            "--webhook-message-direct", type=str, help="Webhook URL for direct/contact messages"
        )
        webhook_group.add_argument(
            "--webhook-message-channel", type=str, help="Webhook URL for channel messages"
        )
        webhook_group.add_argument(
            "--webhook-advertisement", type=str, help="Webhook URL for node advertisements"
        )
        webhook_group.add_argument(
            "--webhook-timeout", type=int, help="Webhook HTTP request timeout in seconds"
        )
        webhook_group.add_argument(
            "--webhook-retry-count", type=int, help="Number of webhook retry attempts on failure"
        )
        webhook_group.add_argument(
        ...
        queue_group.add_argument("--queue-max-size", type=int, help="Maximum command queue size")
        ...
        queue_group.add_argument(
            "--queue-timeout", type=float, help="Maximum time a command waits in queue (seconds)"
        )
        queue_group.add_argument(
            "--rate-limit-per-second", type=float, help="Commands per second (average rate)"
        )
        queue_group.add_argument(
            "--rate-limit-burst", type=int, help="Maximum burst size for rate limiting"
        )
        queue_group.add_argument(
            "--no-rate-limit", action="store_true", help="Disable rate limiting"
        )
        queue_group.add_argument("--debounce-window", type=float, help="Debounce window in seconds")
        queue_group.add_argument(
            "--debounce-cache-size", type=int, help="Maximum debounce cache size"
        )
        queue_group.add_argument(
            "--debounce-commands",
            type=str,
        ...
        queue_group.add_argument("--no-debounce", action="store_true", help="Disable debouncing")
        ...
        if cli_args is None:
            ...
            args = argparse.Namespace(
                **{
                    ...
                    "no_metrics": (
                        cli_args.get("metrics") is False if "metrics" in cli_args else False
                    ),
                    "no_write": (
                        cli_args.get("enable_write") is False
                        if "enable_write" in cli_args
                        else False
                    ),
                    ...
                    "webhook_message_direct_jsonpath": cli_args.get(
                        "webhook_message_direct_jsonpath"
                    ),
                    "webhook_message_channel_jsonpath": cli_args.get(
                        "webhook_message_channel_jsonpath"
                    ),
                    "webhook_advertisement_jsonpath": cli_args.get(
    ...
    def display(self) -> str:
        ...
        if any(
            [self.webhook_message_direct, self.webhook_message_channel, self.webhook_advertisement]
        ):
