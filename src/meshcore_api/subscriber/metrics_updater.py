...
from ..database.models import Node, Message, Advertisement, Telemetry, TracePath, EventLog, NodeTag
...
def update_database_metrics(db_path: str) -> None:
    ...
    try:
        ...
        with session_scope() as session:
            ...
            node_types = session.query(
                Node.node_type,
                func.count(Node.id)
            ).filter(
                Node.last_seen >= one_hour_ago
            ...
            recent_messages = session.query(Message.snr).filter(
                Message.snr.isnot(None)
            ).order_by(Message.received_at.desc()).limit(1000).all()
            ...
            ).group_by(Message.direction, Message.message_type).all()
            ...
            advert_stats = session.query(
                Advertisement.adv_type,
                func.count(Advertisement.id)
            ).group_by(Advertisement.adv_type).all()
            ...
            avg_hops = session.query(func.avg(TracePath.hop_count)).filter(
                TracePath.hop_count.isnot(None)
            ).scalar()
            ...
            # Tag-based metrics
            # Count nodes by area
            area_counts = session.query(
                NodeTag.value_string,
                func.count(func.distinct(NodeTag.node_public_key))
            ).filter(
                NodeTag.key == 'area',
                NodeTag.value_type == 'string'
            ).group_by(NodeTag.value_string).all()
